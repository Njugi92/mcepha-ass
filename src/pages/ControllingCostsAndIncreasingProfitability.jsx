import React from "react";
import "./ControllingCostsAndIncreasingProfitability.css";

const ControllingCostsAndIncreasingProfitability = () => {
  const openLargerMap = () => {
    window.open(
      "https://www.google.com/maps?q=Equity+Plaza,+Thika,+Kenya",
      "_blank"
    );
  };

  return (
    <section className="controlling-costs-container">
      <div className="controlling-costs-title">
        Controlling Costs and Increasing Profitability
      </div>
      <div className="controlling-costs-content">
        <div className="controlling-costs-left">
          <p>
            At Mcepha & Associates, we provide advice on how to control costs
            and increase profitability through various strategies, leaving you
            in a more profitable position.
          </p>
        </div>
        <div className="controlling-costs-right">
          <img
            src="/mcephalogo.png"
            alt="Mcepha & Associates Logo"
            className="logo"
          />
          <div className="map-container">
            <iframe
              title="Thika Equity Plaza"
              src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3167.7407821071495!2d37.06741401544776!3d-1.0410545618454946!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x182f10fd47d62cf9%3A0xacc93d24407a48e5!2sEquity%20Plaza%2C%20Thika!5e0!3m2!1sen!2ske!4v1659557445961!5m2!1sen!2ske"
              width="100%"
              height="300"
              style={{ border: 0 }}
              allowFullScreen=""
              loading="lazy"
            ></iframe>
            <button className="view-larger-map-button" onClick={openLargerMap}>
              View Larger Map
            </button>
            <div className="location-details">
              <p>
                <strong>Mcepha & Associates</strong>
              </p>
              <p>Thika, Equity Plaza Building</p>
              <p>HEAD OFFICE; 5TH FLOOR EQUITY PLAZA</p>
              <p>BIASHARA STREET</p>
              <p>Tel: +254 721-271910/733-997933</p>
              <p>THIKA – KENYA</p>
              <p>
                Emails:{" "}
                <a href="mailto:info.mcepha@gmail.com">info.mcepha@gmail.com</a>
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default ControllingCostsAndIncreasingProfitability;
